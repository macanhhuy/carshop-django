...
#        self.customer = request.user
    ...
    def __init__(self, data=None, files=None, auto_id='id_%s', prefix=None, initial=None, error_class=ErrorList,
        ...
        if instance is not None:
            ...
            if instance.billing_country and instance.billing_country > 0:
            ...
            TEMP_CHOICES = [(u'-1', u'Select...'), ]
            if instance.billing_state and instance.billing_state > 0:
    ...
    billing_country = forms.IntegerField(
        widget=forms.Select(
            attrs={'id': 'id_country', 'class': 'dropdown', 'style': 'width:140px;', 'onchange': 'countryChange(this)'},
            choices=COUNTRY_CHOICES))

    billing_state = forms.IntegerField(
        widget=forms.Select(
            attrs={'id': 'id_state', 'class': 'dropdown', 'style': 'width:120px;', 'onchange': 'stateChange(this)'},
            choices=STATE_CITY_CHOICES))

    billing_city = forms.IntegerField(
        widget=forms.Select(attrs={'id': 'id_city', 'class': 'dropdown', 'style': 'width:100px;'},
                            choices=STATE_CITY_CHOICES))
